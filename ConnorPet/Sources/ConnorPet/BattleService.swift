...
///
///   challenger →  challenge {fromID, fromName, fromPet}
///   accepter   →  accept {fromID, fromName, fromPet, seed}   (accepter picks the seed)
///                 or decline {}
///
/// Once `accept` carries a seed, *both* sides run `simulateBattle(seed:)` and
/// animate the identical result — no further messages, no per-frame sync.
