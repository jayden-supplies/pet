...
- **Claude Code 소스**: 살아있는 세션마다 `sessionId`로 트랜스크립트 경로를 찾습니다(글롭).
- **Orca 소스**: `last-status.json`의 각 항목이 `providerSession.transcriptPath`로 트랜스크립트 경로를 직접 알려주므로 그대로 읽습니다.

경험치 %는 **문맥 창 사용량(context-window usage)** 입니다 — Orca/Claude Code가 구석에 보여주는 "N% 사용"과 같은 값. 즉 **현재 문맥 크기 / 문맥 창(100만)** 입니다. `TranscriptTokenReader`(`TokenUsage.swift`)가 파일 수정시각(mtime) 캐시로, 트랜스크립트의 **마지막** 어시스턴트 메시지 문맥 크기(`input + cache_read + cache_creation`)를 읽습니다(누적 합이 아니라 "지금" 값). 여러 세션이 살아있으면 그중 **가장 큰** 사용량을 씁니다. 문맥 창을 100만으로 잡은 건 실사용 세션(1M 모델)의 "N% 사용"과 맞추기 위해서입니다(문맥 ~80k → 8%).
...
바의 채움은 **다음 진화 단계까지의 진행도**입니다(절대 %가 아니라). 즉 현재 단계의 하한부터 다음 임계치까지를 0~100%로 정규화합니다 — 예를 들어 문맥 8%인데 1단계 임계치가 30%면 바는 `8/30 ≈ 26.7%`만큼 찹니다(`XPModel.barFill`). 마지막 단계에 도달하면 바는 가득 찬 상태로 유지됩니다. **채움 색은 진화 단계**를 나타냅니다 — 0단계=초록, 1단계=파랑, 2단계=골드. 즉 폭은 다음 진화까지의 진행도, 색은 현재 진화 정도를 보여줍니다.

바 위에는 작은 글씨로 **누적 토큰 사용량 라벨**을 함께 그립니다 — `누적 / 최대 (%)` 형식(예: `898M / 100M (898%)`). **누적(분자)** 은 최근 **7일** 동안 `~/.claude/projects`의 모든 트랜스크립트에서 쓴 토큰을 **캐시 읽기까지 전부** 합산한 값이고(`WeeklyUsageReader`, 30초마다 백그라운드에서 mtime 캐시로 재계산), **최대(분모)** 는 메뉴에서 고르는 예산값입니다.

> **감지 가능/불가**: 누적 토큰은 트랜스크립트에서 우리가 직접 계산할 수 있지만, 계정의 **실제 사용 한도(주간 %)** 나 **문맥 창(1M/200k)** 은 디스크에서 신뢰성 있게 읽을 수 없습니다(`~/.claude/policy-limits.json`은 기능 제한 플래그일 뿐). 그래서 최대치(분모)는 자동 감지가 아니라 **사용자가 메뉴에서 지정**하고, 한도 리셋 시점을 알 수 없어 집계는 **롤링 7일**입니다.

### 3. Doc(메뉴바) 아이콘 메뉴 설정

메뉴바 아이콘 메뉴에서 세 가지를 조절합니다(모두 다음 실행에도 복원):

- **"경험치 바 항상 표시"** (체크, **기본 꺼짐**): 끄면(기본) 펫에 **마우스를 올렸을 때만** 바가 나타나고, 켜면 항상 보입니다.
- **"진화 사용"** (체크, 기본 켜짐): 끄면 경험치가 아무리 쌓여도 펫이 **기본 형태**로 고정되고, **경험치 바도 아예 표시되지 않습니다**(스프라이트가 창 전체를 써서 펫이 하단에 붙음). 이때 아래 "진화 % 설정" 항목은 비활성(회색)으로 표시되지만 값은 그대로 보존됩니다.
...
- **"최대 토큰 (라벨): 100M"** (하위 메뉴): 누적 사용량 라벨의 **분모(최대 예산)** 를 프리셋(200k/1M/10M/100M/1B) 중에서 고릅니다. 사람마다 다른 한도를 직접 맞추면 됩니다(캐시 읽기 포함 주간 누적은 쉽게 수백 M이 되므로, %를 100% 이하로 보고 싶으면 1B 등 큰 값을 고르세요).

### 4. 진화 (경험치 %에 따라)

경험치 %가 임계치를 넘으면 펫이 다음 진화형으로 자동 교체됩니다. 사용자가 메뉴에서 고른 **기본 펫은 그대로 유지**되고, 화면에 그려지는 스프라이트만 진화형으로 바뀝니다(진화형은 메뉴에 별도로 노출되지 않습니다).

- **PoC이므로 임계치를 낮게** 잡았습니다: 기본값 **10%**(문맥 ≈100k)에서 1단계, **30%**(≈300k)에서 2단계 진화 (`XPModel.stageThresholds`가 기본값, 실제 값은 위 "진화 % 설정"에서 사용자가 조절).
...
메뉴바에 작은 포켓볼 아이콘이 생깁니다. 클릭하면 위쪽엔 리아코(Totodile)/메타몽(Ditto)/파이리(Charmander)/꼬부기(Squirtle)/꼬마돌(Geodude)/이브이(Eevee)/치코리타(Chikorita)/아차모(Torchic) 중 원하는 펫을, 그 아래엔 상태 소스로 **Claude Code**(기본값)/**Orca** 중 하나를 고를 수 있고(체크 표시가 각각 현재 선택), 그 아래 **"경험치 바 항상 표시"** 토글(기본 꺼짐 — 호버 시에만 표시), **"진화 사용"** 토글(기본 켜짐, 끄면 기본 형태 고정 + 경험치 바 숨김), **"진화 % 설정"** 하위 메뉴(1·2단계 진화 임계 %를 프리셋에서 선택), **"최대 토큰 (라벨)"** 하위 메뉴(누적 사용량 라벨의 분모를 프리셋에서 선택), 맨 아래 Quit으로 종료합니다. 모든 선택이 다음 실행 때도 그대로 복원됩니다. 펫 자체는 화면 우측 하단 근처에 떠서 다른 창들 위에, 모든 Space에서 보이고, 진화가 켜져 있으면 그 아래에 문맥 사용량 경험치 바가 붙습니다. 선택한 소스가 안 깔려있거나 활동 중인 세션/패널이 없으면 그냥 idle 상태로 가만히 있습니다.
