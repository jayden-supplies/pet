...
바는 **다음 진화까지의 진행률**만큼 채워지고, **채움 색은 진화 단계**를 나타냅니다 — 0단계=초록, 1단계=파랑, 2단계=골드. 마우스를 올리면 바 위에 `EXP 15,000,000 / 200,000,000 - 7.50%` 처럼 실제 수치가 뜹니다(분모는 **다음 진화 지점**이라 진화할 때마다 올라가고, 최종 단계에서는 `EXP … - MAX`).
