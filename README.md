...
**파이리만 프레임이 320px 입니다** (`FRAME_BY_PET`). 200px 프레임에는 스프라이트가
164px 들어 있어서 입 왼쪽에 80px 밖에 안 남고, 그보다 긴 불길을 그리면 프레임 밖에서
잘려 단절돼 보입니다. 프레임을 넓혀 300px 가까운 불길이 프레임 안에서 끝나게 했습니다.

프레임이 커지면 같은 크기의 창에 그릴 때 캐릭터가 작아지므로, 앱이 창 크기를 프레임
비율만큼 키웁니다 (`AppDelegate.windowSize(for:)` — 파이리는 144pt, 나머지는 90pt).
**화면에 찍히는 캐릭터 크기는 펫마다 같습니다.** 다만 창이 커진 만큼 펫 주변의
투명한 영역도 넓어지고, 그 영역도 클릭을 받습니다.

불길은 프레임 절대좌표가 아니라 **스프라이트 기준 오프셋**(`MOUTH_IN_SPRITE`)으로
입 위치를 잡고, 그 프레임에서 입 왼쪽에 실제로 남은 공간에 맞춰 크기를 정합니다.
`paste_centered` 가 스프라이트를 프레임 안으로 클램프하면 오프셋이 잘리는데,
`applied_offset` 으로 잘린 뒤의 값을 받아 써야 불길이 입에서 떨어지지 않습니다.
